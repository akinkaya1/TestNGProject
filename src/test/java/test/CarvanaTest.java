...
public class CarvanaTest extends Base{
    ...
    public void validateCarvanaLogo(){
        ...
        WebElement logo = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//a[@aria-label='Carvana Homepage']")));
    ...
    public void validateNavigationHeaderSections(){
        driver.get("https://www.carvana.com/");
        List<WebElement> navigationHeaderSections = driver.findElements(By.xpath("//div[@data-qa='menu-wrapper' and contains(@class, 'style')]"));
    ...
    /**
     ...
     1. "Carvana Certified" inventory type - text should be "Carvana Certified"
    ...
    public void validateResultTiles(){
        ...
        wait.until(ExpectedConditions.urlToBe(expectedUrl));
        Assert.assertEquals(driver.getCurrentUrl(), expectedUrl);
        driver.findElement(By.xpath("//input[@data-qa='search-bar-input']")).sendKeys("mercedes-benz");
        driver.findElement(By.xpath("//button[@data-qa='go-button']")).click();

        wait.until(ExpectedConditions.urlContains("mercedes-benz"));
        ...
        for (int i = 0; i < resultTiles.size(); i++) {
            ...
            Assert.assertEquals(resultTileBodyInventoryTypes.get(i).getText(), "Carvana Certified");
